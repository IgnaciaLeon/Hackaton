...
public interface ServicioRepository extends JpaRepository<Servicio, Long> {
   ...
   // List<Servicio> orderByAmount();
    ...
    Integer sumarMontosDeDescuentoFiltrados(@Param("rut") String rut,
                                            @Param("fecha") Date fecha,
                                            @Param("nombreProducto") String nombreProducto);

/*
    @Query("SELECT t, SUM(t.monto) AS sumaMontos " +
            "FROM TuEntidad t " +
            "GROUP BY t.fecha, t.producto")
    List<Object[]> obtenerSumaMontosPorFechaYProducto();
